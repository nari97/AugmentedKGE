...
class TripleManager():
    ...
    def __init__(self, path, splits, batch_size, neg_rate, use_bern=False, seed=None,
        ...
        if self.corruption_mode == "Global":
            ...
            for r in self.relSet:
                self.headEntities[r] = self.headEntities[-1]
                self.tailEntities[r] = self.tailEntities[-1]
        else:
            for r in self.headDict:
