...
class KG2E(Model):
    ...
    def initialize_model(self):
        ...
        self.register_scale_constraint(emb_type="entity", name="e_mean", p=2)
        self.register_scale_constraint(emb_type="relation", name="r_mean", p=2)
        self.register_custom_constraint(self.hcov_constraint)
        self.register_custom_constraint(self.tcov_constraint)
        self.register_custom_constraint(self.rcov_constraint)

    def hcov_constraint(self, head_emb, rel_emb, tail_emb):
        hcov = head_emb["e_cov"]
        return hcov.clamp(min=self.cmin, max=self.cmax)

    def tcov_constraint(self, head_emb, rel_emb, tail_emb):
        tcov = tail_emb["e_cov"]
        return tcov.clamp(min=self.cmin, max=self.cmax)

    def rcov_constraint(self, head_emb, rel_emb, tail_emb):
        rcov = rel_emb["r_cov"]
        return rcov.clamp(min=self.cmin, max=self.cmax)
    ...
    def _calc(self, h_mean, h_cov, r_mean, r_cov, t_mean, t_cov):
        ...
        if self.variant == 'kl-divergence':
            ...
            scores = .5 * (torch.diagonal(torch.bmm(r_icm, e_cm), dim1=-2, dim2=-1).sum(-1) + \
                torch.bmm(torch.bmm(re_mean.view(batch_size, 1, self.dim), r_icm), re_mean.view(batch_size, self.dim, 1)).flatten() \
