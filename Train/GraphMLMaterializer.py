...
class GraphMLMaterializer(object):
    ...
    def materialize(self, model, name=None):
        ...
        for r in relations.keys():
            ...
            for key, value in neg_triples.items():
                ...
                if value[1]>0 or value[2]>0:
                    if not self.networkx_graph.has_node(key[0]):
                        self.networkx_graph.add_node(key[0], id=key[0])
                    if not self.networkx_graph.has_node(key[1]):
                        self.materialized_networkx_graph.add_node(key[1], id=key[1])
                if value[1] > 0:
                    #materialized_triples_list.append((key[0], r, key[1]))

                    self.networkx_graph.add_edge(key[0], key[1], r=r, triple_type=1)
                    total_materialized += 1
                if value[2] > 0:
                    #mispredicted_triples_list.append((key[0], r, key[1]))
                    self.networkx_graph.add_edge(key[0], key[1], r=r, triple_type=2)
        ...
        return collector, self.networkx_graph
